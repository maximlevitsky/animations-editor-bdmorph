...
template<class T>
void Model2D<T>::changePinnedVertices(set<int> &pinned) {
...
template<class T>
void Model2D<T>::updateCovariance() {
...
void Model2D<T>::recomputeEigenvectors() {
    ...
    qWarning("Known vector computation time: %g", (clock()-t)/(double)CLOCKS_PER_SEC);
    ...
    qWarning("SIZE: %d %d (%d nonzero)", covariance.numRows(), covariance.numCols(), nz);
    qWarning("Nonzero: %d", nz);
    ...
    qWarning("Zero out smallest time: %g", (clock()-t)/(double)CLOCKS_PER_SEC);
    ...
    for (unsigned int i = 3; i < eigenvectors.size(); i++) {
        ...
        double error = 5000;
        int numIterations = 0;
        qWarning("Starting norm: %g", eigenvectors[i].norm());
        while (error > 1e-5) {
            numIterations++;
        ...
        qWarning("Eigenvector %d took %d iterations",i,numIterations);
    }

    double itTime = (clock()-t)/(double)CLOCKS_PER_SEC;
    qWarning("Iteration time: %g", itTime);
}
